"""
Standalone CoLA (Corpus of Linguistic Acceptability) classification test using RoBERTa.

Pipeline:
1) Build HF tokenizer (distilroberta-base) via TokenizerBuilder.
2) Load CoLA with HF collate into train/test DataLoaders.
3) Create RoBERTa classifier via NNModelFactory.
...
def train_and_eval(
    ...
    tokenizer_cfg = {
        "tokenizer": {
            "type": "bert",  # uses HF tokenizer underneath
    ...
    # Convert IterDataPipe to list for reusable dataset if needed
    ...
    console.info(f"Loaded {len(train_dataset_list)} training samples and {len(dev_dataset_list)} dev samples from CoLA")
    ...
    console.info(
        f"Training DistilRoBERTa on CoLA for {epochs} epochs | lr={lr} wd={weight_decay} warmup={warmup_ratio} bs={batch_size} device={device}"
    )
    ...
    console.info("\n" + "="*60)
    console.info("FINAL EVALUATION RESULTS (CoLA - dev):")
...
if __name__ == "__main__":
    metrics = train_and_eval(epochs=30, batch_size=32)
