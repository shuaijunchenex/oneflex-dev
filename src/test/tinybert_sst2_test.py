"""
Standalone SST-2 (Sentiment Analysis) classification test using TinyBERT.
...
def train_and_eval(
	...
	# 2) Data - SST2 returns IterDataPipe which exhausts after 1 iteration
	...
	console.info(f"Loaded {len(train_dataset_list)} training samples and {len(dev_dataset_list)} dev samples from SST-2")
	...
	console.info(
		f"Training TinyBERT on SST-2 for {epochs} epochs | lr={lr} wd={weight_decay} warmup={warmup_ratio} bs={batch_size} device={device}"
	)
	...
	console.info("\n" + "="*60)
	console.info("FINAL EVALUATION RESULTS (SST-2):")
