"""
Standalone QQP (Quora Question Pairs) classification test using TinyBERT.

Pipeline:
1) Build HF tokenizer (bert-base-uncased) via TokenizerBuilder.
2) Load QQP with HF collate into train/test DataLoaders.
...
5) Evaluate on QQP dev set and print metrics.

QQP Task: Binary classification to determine if two questions are semantically equivalent.
"""
...
def train_and_eval(
	...
	# 2) Data - QQP returns IterDataPipe which exhausts after 1 iteration
	...
	console.info(f"Loaded {len(train_dataset_list)} training samples and {len(dev_dataset_list)} dev samples from QQP")
	...
	# QQP format: (label, question1, question2, idx) - use idx_label_text format
	# Need to combine question1 and question2 as "[CLS] q1 [SEP] q2 [SEP]"
	...
	console.info(
		f"Training TinyBERT on QQP for {epochs} epochs | lr={lr} wd={weight_decay} warmup={warmup_ratio} bs={batch_size} device={device}"
	)
	...
	for epoch in range(1, epochs + 1):
		...
		# Early stopping logic based on F1 (common metric for QQP)
		if val_f1 > best_mcc:  # Reusing variable name but tracking F1
			best_mcc = val_f1
			patience_counter = 0
			best_model_state = {k: v.cpu().clone() for k, v in model.state_dict().items()}
			console.info(f"  *** New best F1: {best_mcc:.4f} ***")
		else:
			patience_counter += 1
	...
	console.info("\n" + "="*60)
	console.info("FINAL EVALUATION RESULTS (QQP):")
