"""
Standalone IMDB sentiment classification test using TinyBERT.

Pipeline:
1) Build HF tokenizer (bert-base-uncased) via TokenizerBuilder.
2) Load IMDB with HF collate into train/test DataLoaders.
...
5) Evaluate on IMDB test set and print metrics.
"""
...
def train_and_eval(
	...
	# 2) Data - IMDB torchtext returns IterDataPipe (train/test)
	...
	console.info(f"Loaded {len(train_dataset_list)} training samples and {len(test_dataset_list)} test samples from IMDB")

	from torch.utils.data import DataLoader
	from usyd_learning.ml_data_loader.dataset_loader_util import DatasetLoaderUtil

	# IMDB format: (label, text)
	...
	console.info(
		f"Training TinyBERT on IMDB for {epochs} epochs | lr={lr} wd={weight_decay} warmup={warmup_ratio} bs={batch_size} device={device}"
	)
	...
	console.info("\n" + "="*60)
	console.info("FINAL EVALUATION RESULTS (IMDB - test):")
