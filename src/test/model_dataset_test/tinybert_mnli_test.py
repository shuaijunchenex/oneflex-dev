"""
Standalone MNLI (Multi-Genre Natural Language Inference) classification test using TinyBERT.

Pipeline:
1) Build HF tokenizer (bert-base-uncased) via TokenizerBuilder.
2) Load MNLI with HF collate into train/test DataLoaders.
...
5) Evaluate on MNLI dev set and print metrics.

MNLI Task: 3-way classification (entailment, neutral, contradiction) for sentence pairs.
"""
...
def build_dataloaders(hf_tokenizer, batch_size: int, max_len: int):
	...
	dl_cfg = {
		"data_loader": {
			...
			"is_download": True,
			"train_split": "train",
			"test_split": "dev_matched",  # MNLI has dev_matched and dev_mismatched
			"task_type": "nlp",
		}
	}
...
def build_model(pad_id: int, num_classes: int = 3):
	model_cfg = {
		"nn_model": {
			"name": "tiny_bert",
...
def train_and_eval(
	...
	# 2) Data - MNLI returns IterDataPipe which exhausts after 1 iteration
	...
	console.info(f"Loaded {len(train_dataset_list)} training samples and {len(dev_dataset_list)} dev samples from MNLI")
	...
	# MNLI format: (premise, hypothesis, label) - 3-way classification
	...
	model = build_model(pad_id, num_classes=3).to(device)  # 3-way classification
	...
	console.info(
		f"Training TinyBERT on MNLI for {epochs} epochs | lr={lr} wd={weight_decay} warmup={warmup_ratio} bs={batch_size} device={device}"
	)
	...
	for epoch in range(1, epochs + 1):
		...
		# Early stopping logic based on accuracy (standard for MNLI)
		if val_acc > best_acc:
			best_acc = val_acc
			patience_counter = 0
	...
	console.info("\n" + "="*60)
	console.info("FINAL EVALUATION RESULTS (MNLI - dev_matched):")
